...
def update_readme_version(version):
    if not os.path.exists(README_PATH): return False
    ...
    if count > 0:
        ...
        log_and_print(f"README.md updated to v{version}", "INFO")
        return True
    return False
...
def handle_deploy(version):
    print(f"\n--- STARTING FULL DEPLOY v{version} ---")
    ...
    if os.path.exists(STAGING_DIR):
        ...
        create_zip(STAGING_DIR, bin_zip)
    else:
        log_and_print(f"WARNING: Staging dir {STAGING_DIR} not found. No bin assets.", "ERROR")

    # 2. Shield Dependencies
    ...
    if deps_exist:
        ...
        log_and_print("Dependencies shielded.", "INFO")

    try:
        # 3. Orphan master update
        ...
        check_run(f'gh release create v{version} --repo {repo} --title "v{version}" --generate-notes')
        
        if os.path.exists(bin_zip):
            check_run(f'gh release upload v{version} {bin_zip} --repo {repo}')
            os.remove(bin_zip)
